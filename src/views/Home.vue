              ...
              <td>
                <div class="d-flex">
                  <button
                    ...
                    :disabled="checkDisableEdit(user.customerId)"
                  >
                    Edit
                  </button>
                              ...
                              <div class="d-flex">
                                ...
                                >
                                  <select
                                    class="custom-select "
                                    id="emailPart2"
                                    v-model="editUserData.emailPart2"
                                  >
                                    <option selected>
                  ...
                  <button
                    ...
                    :disabled="checkDisableDelete(user.customerId)"
                  >
                    Delete
                  </button>
                </div>
...
export default {
  ...
  methods: {
    checkDisableEdit(id) {
      if (
        this.idUser === id.toString() ||
        this.idUser === this.ADMIN_ID.toString()
      ) {
        return false;
      } else {
        return true;
    ...
    checkDisableDelete(id) {
     
      if (this.idUser === this.ADMIN_ID) {
        return false;
      } else {
        return true;
      }
    ...
    async submitEditForm() {
      ...
      const payloadEdit = {
        ...
        phoneNumber:
          user.phone_number_part1 +
          user.phone_number_part2 +
          user.phone_number_part3,
    ...
    openEditModal(user) {
      ...
      console.log(
        "🚀 ~ file: Home.vue:530 ~ openEditModal ~ payloadEdit:",
        payloadEdit
      );
    ...
    updateDateRange(selectedRange) {
      console.log(
        "🚀 ~ file: Home.vue:578 ~ updateDateRange ~ selectedRange:",
        selectedRange
      );
    ...
    async deleteUser(id) {
      ...
      try {
        ...
        console.log("response", response);
        this.loadPage(1);
        this.infoMessage = response.data.data;
      } catch (error) {
        if (!error.response) {
          this.errorMessage = error.response.data.message;
        } else if (error.response.status === 409) {
    ...
    async handleSearch() {
      ...
      console.log(
        "🚀 ~ file: Home.vue:689 ~ handleSearch ~ payload:XXXXXXXXXX",
        payload
      );
      ...
      try {
        ...
        if (!isPayloadEmpty && storedResponse) {
          ...
          console.log("🚀 ~ file: Home.vue:389 ~ handleSearch ~ ", response);
        }
    ...
    async exportToExcel() {
      const storedResponse = JSON.parse(localStorage.getItem("response"));
      console.log(
        "🚀 ~ file: Home.vue:735 ~ exportToExcel ~ storedResponse:",
        storedResponse
      );
      console.log("test EXPORT");
      console.log("search", this.searchPayload);
    ...
    validateId() {
      // Check if ID is at least 3 digits long
      this.isIdValid = /^[0-9]{3,}$/.test(this.searchForm.customer_id);
    },
    validateName() {
      ...
      this.isNameValid =
        /^[A-Za-z ]+$/.test(enteredText) && enteredText.length > 1;
    },
  ...
  mounted() {
    ...
    // Set the default date range to the searchForm
    if (!this.searchForm.rangeTime || this.searchForm.rangeTime.length === 0) {
      ...
      const endDate = moment()
        .endOf("day")
        .toDate();
      this.searchForm.rangeTime = [startDate, endDate];
    }
    this.idUser = JSON.parse(localStorage.getItem("currentUser"));

    console.log("check id User", this.idUser);
  },
};
</script>
